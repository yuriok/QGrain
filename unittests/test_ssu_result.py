...
class TestSSUResult:
    ...
    def test_none_error(self):
        ...
        with pytest.raises(AssertionError):
            SSUResult(None, self.dataset.distribution_type, parameters[0], parameters, 1.0, settings={})
        with pytest.raises(AssertionError):
            SSUResult(self.dataset[0], None, parameters[0], parameters, 1.0, settings={})
        with pytest.raises(AssertionError):
            SSUResult(self.dataset[0], self.dataset.distribution_type, None, parameters, 1.0, settings={})
        with pytest.raises(AssertionError):
            SSUResult(self.dataset[0], self.dataset.distribution_type, parameters[0], None, 1.0, settings={})
        with pytest.raises(AssertionError):
            SSUResult(self.dataset[0], self.dataset.distribution_type, parameters[0], parameters, None, settings={})

    def test_ndim_error(self):
        parameters = self.dataset.parameters.copy()
        with pytest.raises(AssertionError):
            SSUResult(self.dataset[0], self.dataset.distribution_type, parameters[0], parameters[0], 1.0, settings={})

    def test_n_parameters_error(self):
        parameters = self.dataset.parameters.copy()
        with pytest.raises(AssertionError):
            SSUResult(self.dataset[0], DistributionType.Normal, parameters[0], parameters, 1.0, settings={})
