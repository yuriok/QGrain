...
class TestSSUResult:
    ...
    def test_ctor(self):
        as0 = self.dataset[0]
        result = SSUResult(as0, self.dataset.distribution_type, self.dataset.parameters, 1.0)
    ...
    def test_none_error(self):
        ...
        with pytest.raises(AssertionError):
            SSUResult(self.dataset[0], self.dataset.distribution_type, None, 1.0)
        with pytest.raises(AssertionError):
            SSUResult(self.dataset[0], self.dataset.distribution_type, parameters, None)

    def test_ndim_error(self):
        parameters = self.dataset.parameters.copy()
        with pytest.raises(AssertionError):
            SSUResult(self.dataset[0], self.dataset.distribution_type, parameters[0], 1.0)

    def test_n_parameters_error(self):
        parameters = self.dataset.parameters.copy()
        with pytest.raises(AssertionError):
            SSUResult(self.dataset[0], DistributionType.Normal, parameters, 1.0)
