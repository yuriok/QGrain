...
class EMMAResolver:
    ...
    def try_fit(self, dataset: GrainSizeDataset,
                kernel_type: KernelType,
                n_members: int,
                resolver_setting: EMMAResolverSetting=None,
                params=None,
                update_end_members = True):
        if resolver_setting is None:
            setting = EMMAResolverSetting(max_niter=1000, tol=1e-4, ftol=1e-6, lr=1e-2)
        else:
            assert isinstance(resolver_setting, EMMAResolverSetting)
        ...
        optimizer = torch.optim.Adam(emma.parameters(), lr=setting.lr)
        loss_series = []
        history = []
        iteration = 0
        start = time.time()
    ...
    def get_params(self, dataset: GrainSizeDataset,
        ...
        if kernel_type == KernelType.Nonparametric:
            ...
            def get_x0(sample: GrainSizeSample):
                ...
                return x0
            resolver_setting = SSUResolverSetting(try_GO=True, GO_max_niter=500, GO_success_niter=10, GO_step=0.1)
